...
class Page p => FormPageView p where
...
data FormPage p = FormPage (View (Html ())) ST (Frame p)

instance Page p => Page (FormPage p) where
    isPrivatePage (FormPage _v _a p) = isPrivatePage p
    extraPageHeaders (FormPage _v _a p) = extraPageHeaders p

instance FormPageView p => ToHtml (FormPage p) where
    toHtmlRaw = toHtml
    toHtml fop@(FormPage v a frp) = frameToHtml $ formPage v a <$> frp
      where
  ...
  where
    ...
    postH formData = do
        ...
        (case mpayload of
            ...
            Nothing      -> FormPage v fa <$> makeFrame page)
