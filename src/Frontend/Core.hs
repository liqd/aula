...
semanticDiv' attrs t = div_ $ semanticDivAttr t : attrs

semanticDivAttr :: forall a . Typeable a => a -> Attribute
...
data ServerErrorPage = Page404 | Page5xx
  deriving (Eq, Show)

instance Page ServerErrorPage where
    isAuthorized = publicPage

instance ToHtml ServerErrorPage where
    toHtmlRaw = toHtml
...
form formHandler = getH :<|> postH
  where
    ...
    getH = runHandler $ do
        ...
        t <- getCsrfToken
        logEvent DEBUG $ "render form " <> cshow (typeOf page)
